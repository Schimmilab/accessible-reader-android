...
Bookmarks provide section starts. Without bookmarks, `groupPagesIntoSections` collects pages into sections of about ten thousand characters, which is roughly ten minutes of listening. One section per page was the earlier behaviour and it listened badly: a book of four hundred pages then had four hundred interruptions, each with its own spoken section number. The opening of the next section is synthesized into the audio cache while the current one still plays, so the change itself costs well under a second with the stock voices, against nine seconds without it. The app does not guess headings. Pages without text are reported, password protection and text extraction that is not allowed lead to an understandable error message.
