...
Bookmarks provide section starts. Without bookmarks, `groupPagesIntoSections` collects pages into sections of about ten thousand characters, which is roughly ten minutes of listening. One section per page was the earlier behaviour and it listened badly: playback stops for eight to twelve seconds at the end of every section while the next one is synthesized, so a book of four hundred pages paused after every page. The app does not guess headings. Pages without text are reported, password protection and text extraction that is not allowed lead to an understandable error message.
