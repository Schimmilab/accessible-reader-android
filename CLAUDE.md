...
- Two `TextToSpeech` instances against one engine is fine for Google, Vocalizer and Acapela, but an engine with a single native synthesizer serves them one after the other. That is why an announced chapter switch was far slower than a silent one on the neural engine.
